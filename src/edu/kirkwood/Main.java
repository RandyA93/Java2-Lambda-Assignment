...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        UnaryOperator<Widget> nameToLower = widget -> {
            ...
            String updatedName = widgetName.toLowerCase();
            widget.setName(updatedName);
            return widget;
        };

        UnaryOperator<Widget> nameToUpper = widget -> {
            String widgetName = widget.getName();
            String updatedName = widgetName.toUpperCase();
            widget.setName(updatedName);
            return widget;
        };
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (zeroOrGreater.test(widget)) {
                ...
                nameToUpper.apply(widget);
            }
            // STEP 7: add the widget to the array
            for (int u = 0; u < widgets.length; u++) {
                if (widgets[u] == null) {
                    widgets[u] = widget;
                    break;
                }
